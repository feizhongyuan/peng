...
public class WaterRippleView extends View {
    ...
    private int backGroundColor = R.color.calendar_header;//背景颜色
    private int shadowColor = Color.argb(120, 2, 199, 46);//阴影颜色
    private int frontColor = Color.argb(255, 2, 255, 46);//正面颜色
    ...
    public WaterRippleView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mPaint = new Paint(Paint.ANTI_ALIAS_FLAG);//正面画笔
        mPaint.setAntiAlias(true);
        mPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
        mPaint.setColor(frontColor);
        mPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_ATOP));
        mPaint2 = new Paint(Paint.ANTI_ALIAS_FLAG);//阴影画笔
        ...
        mBgPaint = new Paint(Paint.ANTI_ALIAS_FLAG);//背景画笔
...
//    持续动画
    ...
    private void calculatePath() {
        ...
        for (float x = 0; x <= mWidth; x = x + 1) {
            y1 = (float) (mHeight * 0.06f * (1 - Math.abs(mHeight * mFraction - 0.5f * mHeight) / (0.5f * mHeight))
                    * Math.sin(Math.toRadians(90 / (mWidth / 3) * x + 720 * mFraction3))) + mHeight * mFraction;
            mPath.lineTo(x, y1);
        }
        for (float x = 0; x <= mWidth; x = x + 1) {
            y2 = (float) (mHeight * 0.06f * (1 - Math.abs(mHeight * mFraction - 0.5f * mHeight) / (0.5f * mHeight))
                    * Math.sin(Math.toRadians(90 / (mWidth / 3) * x + 720 * mFraction3 + 180))) + mHeight * mFraction;
            mPath2.lineTo(x, y2);
        }
